...
use nvidia::caps::{CudaDecodeSpec, get_decode_capabilities, get_encode_capabilities};
...
pub fn get_nvidia_devices() -> Result<Vec<Device>, NvidiaError> {
    ...
    for device in devices {
        ...
        })?.into_iter().for_each(|codec_details| {
        ...
        result.push(Device::new_with_ordinal(Driver::Nvidia, device.handle as u8, device.name, codec_details))
    }
